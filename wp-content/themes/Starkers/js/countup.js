...
(function ( $ ) {
	...
	var _lang = { 
		'en' : { 'year': 'years', 'month': 'months', 'day': 'days', 'hour': 'hours', 'minute': 'min', 'seconds': 'secs',  },
		'tr' : { 'year': 'yÄ±l', 'month': 'ay', 'day': 'gÃ¼n', 'hour': 'saat', 'minute': 'dakika', 'seconds': 'saniye' },
		'es' : { 'year': 'aÃ±o' ,'month': 'mes', 'day': 'dÃ­a' ,'hour': 'horas' ,'minute': 'minuto','seconds': 'segundo'	},
	...
	var methods = {
		...
		init : function (options) {
			...
			var key = '__' + _defaults['div_id'] + '_intervalId';
			var intervalId = $(window).data(key);
			
			function refreshTimer(_key, _timeDiff){
				return function(){
					console.log(_key+": "+_timeDiff.hour+':'+_timeDiff.minute+':'+_timeDiff.seconds);
					methods.update(_timeDiff, newOptions);
				};
			}
			if(intervalId != undefined){
				clearInterval(intervalId);
			} 
			intervalId = setInterval(refreshTimer(key, _timeDifference), 999);
			$(window).data(key, intervalId);
		},
		...
		print : function(time, newOptions) {
			...
			//var year   = '<span id='+newOptions["class_prefix"]+'_year"><strong>'+time.year+'</strong> '+_lang[newOptions['lang']].year+newOptions['seperator']+' </span>';
			//var month  = '<span id='+newOptions["class_prefix"]+'_month"><strong>'+time.month+'</strong> '+_lang[newOptions['lang']].month+newOptions['seperator']+' </span>';
			//var day    = '<span id='+newOptions["class_prefix"]+'_day"><strong>'+time.day+'</strong> '+_lang[newOptions['lang']].day+newOptions['seperator']+' </span>';
			var hour   = '<span id='+newOptions["class_prefix"]+'_hour" class="hrs"><strong class="hour-num">'+time.hour+'</strong> '+_lang[newOptions['lang']].hour+newOptions['seperator']+' </span>';
			var minute = '<span id='+newOptions["class_prefix"]+'_minute" class="mins"><strong class="mins-num">'+time.minute+'</strong> '+_lang[newOptions['lang']].minute+newOptions['seperator']+' </span>';
			var seconds = '<span id='+newOptions["class_prefix"]+'_seconds" class="secs"><strong class="secs-num">'+time.seconds+'</strong> '+_lang[newOptions['lang']].seconds+' </span>';
