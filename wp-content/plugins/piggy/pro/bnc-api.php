...
require_once( PIGGY_DIR . '/include/bnc-api-functions.php' );

//! The current version of the BNC API
define( 'BNC_PIGGY_API_VERSION', '1.3' );

//! The URL used to access the BNC API
define( 'BNC_PIGGY_API_URL', 'http://api.bravenewcode.com/v/' . BNC_PIGGY_API_VERSION );

//! The WordPress setting name for the token information 
define( 'BNC_PIGGY_TOKEN_INFO_SETTING', 'bncid_piggy_token' );

define( 'BNC_PIGGY_AUTHENTICATION_ATTEMPTS', 2 );

define( 'BNC_PIGGY_API_TIMEOUT', 8 );

define( 'BNC_PIGGY_PRODUCT_NAME', 'piggy' );

define( 'PIGGY_DEBUG_WARNING', 0 );
define( 'PIGGY_DEBUG_INFO', 1 );
define( 'PIGGY_DEBUG_ERROR', 2 );

function PIGGY_DEBUG( $type, $message ) {
...
class PiggyBNCAPI {
	...
	function PiggyBNCAPI( $bncid, $license_key ) {
		$this->bncid = $bncid;
		...
		$token_info = get_option( BNC_PIGGY_TOKEN_INFO_SETTING, false );
		if ( $token_info ) {
	...
	function do_api_request( $method, $command, $params = array(), $do_auth = false ) {
		$url = BNC_PIGGY_API_URL . "/{$method}/{$command}/";
        ...
        $options = array( 'method' => 'POST', 'timeout' => BNC_PIGGY_API_TIMEOUT, 'body' => $body );
        $options['headers'] = array(
            'Content-Type' => 'application/x-www-form-urlencoded; charset=' . get_option('blog_charset'),
            'Content-Length' => strlen( $body ),
            'User-Agent' => 'WordPress/' . get_bloginfo("version") . '/Piggy',
        ...
        if ( !is_wp_error( $raw_response ) ) {
        	...
        	if ( $raw_response['response']['code'] == 200 ) {
        		...
        		PIGGY_DEBUG( PIGGY_DEBUG_WARNING, "Unable to connect to server. Response code is " . $raw_response['response']['code'] );
        	}
        } 
	...
	function get_product_list() {
		if ( $this->internal_check_token() ) {
			...
			$params = array(
				'bncid' => $this->bncid,
				'product_name' => BNC_PIGGY_PRODUCT_NAME
			);
	...
	function authenticate() {
		PIGGY_DEBUG( PIGGY_DEBUG_ERROR, "Authenticating" );
		...
		if ( $nonce ) {
			...
			$params['product_name'] = BNC_PIGGY_PRODUCT_NAME;
			...
			if ( $auth_result ) {
				if ( $auth_result['status'] == 'ok' ) {
					...
					update_option( BNC_PIGGY_TOKEN_INFO_SETTING, $token_info );
	...
	function invalidate_all_tokens() {
		delete_option( BNC_PIGGY_TOKEN_INFO_SETTING );	
	}
	...
	function check_token() {
		...
		if ( $this->token ) {
			...
			PIGGY_DEBUG( PIGGY_DEBUG_ERROR, "Performing server token check" );
	...
	function try_multiple_authentications() {
		...
		while ( true ) {
			...
			if ( $attempts > BNC_PIGGY_AUTHENTICATION_ATTEMPTS ) {
				break;	
			}	
		}	
	...
	function internal_check_token() {
		...
		if ( time() > ( $this->token_expires - 15 ) ) {
			PIGGY_DEBUG( PIGGY_DEBUG_INFO, "Authenticating due to expiry" );
		...
		if ( $succeeded && !$this->token_checked ) {	
			...
			if ( ( time() - $this->last_token_check ) > 300 ) {	
				PIGGY_DEBUG( PIGGY_DEBUG_INFO, 'Forcing token check due to cache expiry' );
	...
	function get_license_reset_info() {
		if ( $this->internal_check_token() ) {		
			...
			PIGGY_DEBUG( PIGGY_DEBUG_INFO, "Getting license reset information" );
